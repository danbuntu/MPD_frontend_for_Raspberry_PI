	...
	class mpdEventListener {
		...
		public function startListening() {
			...
			do {
				...
				$this->outputs = $this->mpd->GetOutputs();
				sleep($this->checkTime);
				$this->mpd->RefreshInfo();
				$this->_checkStatus($this->mpd->GetStatus(), $this->mpd->GetPlaylist(), $this->mpd->GetOutputs());
			} while (true);
		}
		...
		private function _checkStatus($newStatus, $newPlaylist, $newOutputs) {
			$change_status = array_diff_assoc($newStatus,$this->status);
			foreach ($this->outputs as $key => $output) {
				$change_outputs[] = array_diff_assoc($newOutputs[$key],$output); 
			}
			
			foreach ($change_outputs as $key => $output) {
				foreach ($output as $attr => $value) {
					$this->_triggerEvent(MPDEVENTLISTENER_ONOUTPUTCHANGE,$this->outputs[$key],$newOutputs[$key]);
				}
			}
			...
			foreach ($change_status as $attr => $value) {
				switch ($attr) {
					...
					case 'state':
						...
						break;
					case 'songid':
						$this->_triggerEvent(MPDEVENTLISTENER_ONSONGCHANGE,$this->mpd->GetSong($this->status['songid']),$this->mpd->GetSong($value));
						break;
					case 'time':
